...
def undo_highlight(event):
    ...
    if highlight_history:
        last_highlight = highlight_history.pop()
        last_highlight.remove()
        highlights.remove(last_highlight)
        fig.canvas.draw_idle()
...
scatter2 = ax1.scatter(landmark1[:, 0], landmark1[:, 1], s=30, c="red", picker=True, pickradius=10)
ax1.invert_yaxis()
ax1.invert_xaxis()
...
# 添加清除高亮的按钮
clear_button_ax = plt.axes([0.9, 0.05, 0.1, 0.05])
clear_button = Button(clear_button_ax, 'Clear Highlights', hovercolor='0.9')
clear_button.on_clicked(clear_highlights)

# 添加撤销按钮
undo_button_ax = plt.axes([0.9, 0.1, 0.1, 0.05])
undo_button = Button(undo_button_ax, 'Undo Highlight', hovercolor='0.9')
undo_button.on_clicked(undo_highlight)

# 添加保存按钮
save_button_ax = plt.axes([0.9, 0.15, 0.1, 0.05])
